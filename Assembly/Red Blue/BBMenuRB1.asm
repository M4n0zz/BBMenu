...
pop  de                       ; $d66a
ld   hl, .end                 ; origin right after installer
call CopyMapConnectionHeaderloop

; move OAM hijack + BBMenu Loader + scripts into SRAM Bank 1
ld   c, sramlength
ld   de, sramcopy             ; destination at a000
call CopyMapConnectionHeaderloop
...
; closes SRAM
call CloseSRAM                ; it also enables interrupts (needed only in RB)
...
; Loaded from SRAM1 $bb80 to $da81 automatically with map script pointer
...
call LoadTileBlockMap         ; reload map after closing BBMenu
